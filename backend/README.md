...
| **Post** | `api::post.post` | Public marketing blog articles, category anchor pieces (`isAnchor`), highlighted pinned articles (`isPinned`), and service distinction (`serviceType: 'construction' \| 'keyholding'`). | Many-to-Many `categories`, Many-to-One `author`. |
| **Category** | `api::category.category` | Service categories and blog tags. | Many-to-Many `posts`. |
| **Author** | `api::author.author` | Blog article authors and team members. | One-to-Many `posts`. |
